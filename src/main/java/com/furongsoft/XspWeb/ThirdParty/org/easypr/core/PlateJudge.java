...
public class PlateJudge
{
    ...
    {
        svm.clear();
        svm.load(ApplicationContextHolder.getResourcePath(s), "svm");
    }
    ...
    public int plateJudge(Vector<Mat> inVec, Vector<Mat> resultVec)
    {
        ...
        for (int j = 0; j < inVec.size(); j++) {
            ...
            if (1 == plateJudge(inMat)) {
                ...
                Mat tmpMat = new Mat(inMat, new Rect(
                    (int) (w * 0.05), (int) (h * 0.1), (int) (w * 0.9),
                    (int) (h * 0.8)));
     ...
     * @return
     */
    public int plateJudge2(
        Vector<Mat> inVec,
        Vector<Mat> resultVec,
        Vector<RotatedRect> inOriginalVec,
        Vector<RotatedRect> resultOriginalVec)
    {
        for (int j = 0; j < inVec.size(); j++) {
            Mat inMat = inVec.get(j);
            RotatedRect rect = inOriginalVec.get(j);

            if (1 == plateJudge(inMat)) {
                resultVec.add(inMat);
                resultOriginalVec.add(rect);
            }
            else { // 再取中间部分判断一次
                int w = inMat.cols();
                int h = inMat.rows();
     ...
     * 模型存储路径
     */
    private String path = "classpath:EasyPr/res/model/svm.xml";
}
