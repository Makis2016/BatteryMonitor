...
<body>
<div id="addBatteryGroupHtml" style="display: block;overflow: hidden;">
    <div style="width: 460px;height: 945px;float: left;">
        <div id="batteryPack_div" style="width: 450px;height: 200px;float: left;margin-top: 25px;margin-left: 50px;">
            <div class="batteryPack_title" style="margin-bottom: 3px;">
                电池组信息<a id="exportConfig" style="font-size: 12px;text-decoration: none;" href="javascript:void(0);">
                导出配置</a><%--<span>&nbsp;ECU版本：</span><span id="ECUVN">VB100A320</span>--%>
            </div>
            ...
            <div style="width: 450px;height: 30px;">
                ...
                <div style="float: left;">
                    <input test="true" onchange="batteryPackInputOnChangeEvent(this)" id="batteryCount" type="text"
                           style="width: 200px;height: 19px;line-height: 19px;" disabled="disabled"/></div>
            ...
            <div style="width: 450px;height: 30px;float: left;">
                ...
                <div style="float: left;margin-top: 2px;">
                    ...
                    <input id="battery_characteristicCurve_import" onclick="DownloadCharacter()" type="button"
                           value="导出"
                           style="color: #ffffff;width: 100px;height: 22px;line-height: 22px;background-color: #2397d3"/>
                </div>
            </div>
        ...
        <div id="alarm_div" style="width: 450px;height: 687px;float: left;margin-top: 25px;margin-left: 50px;">
...
<div style="text-align: center;float:left;width:100%;height: 40px;">
    <input type="button" id="btnSure" value="确定" class="btn0"/>
</div>
...
<script>
    var parameters = {
        Action: {
            editGridBatteryPack: "${_RootPath}/System/editBatteryPack"
        },
        batteryPackParams: null,
        arrBattery: null,
        battery: null,
        arrNFNByBatteryPackID: [{id: "batteryPack_name", name: "电池组名称"},
        ...
        arrDefaultByBatteryPackID: [
            ...
            {id: "voltageRange"},
            {id: "checkCycleTime"},
            {id: "battery_characteristicCurve"},//6
            {id: "groupVoltageUp"},
            {id: "groupVoltageDown"},
            {id: "envirTeaperature"},
            ...
            {id: "batteryTeamperatureLevelBaseLine1"},//14
            {id: "batteryTeamperatureLevelBaseLine2"},
        ...
        arrDefaultByBatteryID: [
            ...
            {id: "chargeRate"},
            {id: "schedulingCycle"},
            {id: "voltageThresholdUp"},//index = 19
    ...
    // 设置电池组端编辑信息
    function setBatteryPackEditInfo() {
        var batteryPackInfo = ${batteryPackInfo};
        parameters.batteryPackParams = {};
        if (batteryPackInfo) {
            parameters.batteryPackParams.batteryPack_id = batteryPackInfo.id;

            var batteryPackParmIds = parameters.arrDefaultByBatteryPackID;
            for (var i = 0; i < batteryPackParmIds.length; i++) {
                if (batteryPackParmIds[i].id == "batteryPack_name") {
                    parameters.batteryPackParams.batteryPack_name = batteryPackInfo.name;
                    $("#batteryPack_name").val(batteryPackInfo.name);
                } else if (batteryPackParmIds[i].id == "batteryCount") {
                    parameters.batteryPackParams.batteryCount = batteryPackInfo.count;
                    $("#batteryCount").val(batteryPackInfo.count);
                    setBatteryEditInfo(batteryPackInfo.count);
                } else if (i > 6 && i < 14 && batteryPackInfo.thresholdConfig) {
                    var AlarmThreshold = JSON.parse(batteryPackInfo.thresholdConfig);
                    var index = batteryPackParmIds[i].id;
                    $("#" + index).val(AlarmThreshold[index]);
                    parameters.batteryPackParams[index] = AlarmThreshold[index];
                } else {
                    var index = batteryPackParmIds[i].id;
                    parameters.batteryPackParams[index] = batteryPackInfo[index];
                    if ((index === "type") && (batteryPackInfo[index])) {
                        $("#" + index).combobox('select', batteryPackInfo[index]);
                    }
                    else if (index !== "battery_characteristicCurve") {
                        $("#" + index).val(batteryPackInfo[index]);
                    }
                }
            }

            var batteriesInfo = ${batteriesInfo};
            if (batteriesInfo && batteriesInfo.length > 0 && batteriesInfo["0"].thresholdConfig) {
                var batteryParmIds = parameters.arrDefaultByBatteryID;
                for (var i = 19; i < batteryParmIds.length; i++) {
                    var batteryAlarmThreshold = JSON.parse(batteriesInfo["0"].thresholdConfig);
                    var index = batteryParmIds[i].id;
                    $("#" + index).val(batteryAlarmThreshold[index]);
                    parameters.batteryPackParams[index] = batteryAlarmThreshold[index];
                }
            }
        }
    }
    ...
    // 设置电池组单体编辑信息
    function setBatteryEditInfo(count) {
        createBatteyTableBtn(count < 60 ? 60 : count);
        parameters.arrBattery = new Array(count);
        var batteriesInfo = ${batteriesInfo};
        // console.log(batteriesInfo);
        if (batteriesInfo && batteriesInfo.length > 0)
            for (var i = 0; i < count; i++) {
                parameters.arrBattery[i] = {};
                parameters.arrBattery[i].no = batteriesInfo[i].id;
                parameters.arrBattery[i].isAlarm = batteriesInfo[i].alarm;
                var batteryAlarmTh = JSON.parse(batteriesInfo[i].thresholdConfig);
                for (var j = 0; j < parameters.arrDefaultByBatteryID.length; j++) {
                    var objBattery = parameters.arrDefaultByBatteryID[j];
                    parameters.arrBattery[i][objBattery.id] = batteriesInfo[i][objBattery.id];
                    // 单体告警阀值
                    if (j > 18) {
                        parameters.arrBattery[i][objBattery.id] = batteryAlarmTh[objBattery.id];
                    }
                }
            }
    ...
    function DownloadCharacter() {
        var c = ${batteriesInfo};
        var id = c[0].battery_pack_id;
        XspWeb.Misc.Download("${_RootPath}/System/exportCharacterConfig", {
            id: id
        });
    }
    ...
    function seletedBattery(j, count) {
        // $("#battery_title").html(j + "号电池信息&nbsp;"+'<span>BCU版本：V'+j+'00BCU0</span>');
    ...
    function setBatteryInputVal(battery) {
        if (battery) {
            ...
            if (battery.battery_specifications) {
        ...
        if (parameters.arrBattery && parameters.arrBattery.length > 0) {
            for (var j = 0; j < parameters.arrBattery.length; j++) {
                ...
                for (var i = 0; i < parameters.arrNFNByBatteryID.length; i++) {
                    ...
                    if ((!batteryObj[obj.id] && batteryObj[obj.id] !== 0) || (batteryObj[obj.id] === "")) {
                        return (j + 1) + "号电池信息不完整：" + obj.name + "不可为空！";
                    }
                }
            }
            return null;
    ...
    function editBatteryPack() {
        ...
        XspWeb.Misc.Ajax({
            action: parameters.Action.editGridBatteryPack,
            data: {
            ...
            success: function (rec) {
                if (rec.code === 0) {
                    ...
                    if (typeof XspWeb.Controls.Dialog.Callbacks.editBatteryGroupSuccess === "function")
                        XspWeb.Controls.Dialog.Callbacks.editBatteryGroupSuccess();
                } else {
                    XspWeb.Misc.Tips(rec.message);
                }
    ...
    function exportBatteryPackConfig() {
        var arrBatteryJson = JSON.stringify(parameters.arrBattery);
        // console.log(parameters.arrBattery);
        var batteryPackParamsJson = JSON.stringify(parameters.batteryPackParams);
        XspWeb.Misc.Download("${_RootPath}/System/exportBatteryPackSettingConfig", {
            arrBatteryJson: arrBatteryJson,
            batteryPackParamsJson: batteryPackParamsJson
        });
    }
    ...
    function bindBtnEvent() {
        $("#btnSure").click(editBatteryPack);
        $("#exportConfig").click(exportBatteryPackConfig);
    }
