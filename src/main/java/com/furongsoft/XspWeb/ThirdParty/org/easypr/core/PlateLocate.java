package com.furongsoft.XspWeb.ThirdParty.org.easypr.core;
...
import com.furongsoft.XspWeb.ThirdParty.org.easypr.util.DebugHelper;
...
public class PlateLocate
{
    ...
    public Vector<Mat> plateLocate(Mat src)
    {
        ...
        GaussianBlur(
            src, src_blur, new Size(gaussianBlurSize, gaussianBlurSize), 0, 0,
            BORDER_DEFAULT);
        if (debug) {
        ...
        addWeighted(
            abs_grad_x, SOBEL_X_WEIGHT, abs_grad_y, SOBEL_Y_WEIGHT, 0, grad);
        ...
        threshold(
            grad, img_threshold, 0, 255, CV_THRESH_OTSU + CV_THRESH_BINARY);
        ...
        for (int i = 0; i < rects.size(); i++) {
            ...
            if (verifySizes(minRect)) {
                ...
                if (debug) {
                    ...
                    for (int j = 0; j < 4; j++) {
                        Point pt1 = new Point(rect_points
                            .position(j).asCvPoint2D32f());
                        Point pt2 = new Point(rect_points
                            .position((j + 1) % 4).asCvPoint2D32f());

                        line(
                            result, pt1, pt2, new Scalar(0, 255, 255, 255), 1,
                            8, 0);
                    }
                }
                ...
                Size rect_size = new Size(
                    (int) minRect.size().width(), (int) minRect.size().height());
                if (r < 1) {
                    angle = 90 + angle;
                ...
                if (angle - this.angle < 0 && angle + this.angle > 0) {
                    ...
                    Mat resultMat = showResultMat(
                        img_rotated, rect_size, minRect.center(), k++);
     ...
     * @return 一个Mat的向量，存储所有抓取到的图像
     */
    public Vector<Mat> plateLocate2(Mat src, Vector<RotatedRect> originalVec)
    {
        Vector<Mat> resultVec = new Vector<Mat>();
        ...
        if (debug) {
            imwrite(
                DebugHelper.getBasePath() + "/tmp/debug_GaussianBlur.jpg",
                src_blur);
        }
        ...
        cvtColor(src_blur, src_gray, CV_RGB2GRAY);
        if (debug) {
            imwrite(DebugHelper.getBasePath() + "/tmp/debug_gray.jpg", src_gray);
        }
        ...
        if (debug) {
            imwrite(DebugHelper.getBasePath() + "/tmp/debug_Sobel.jpg", grad);
        }
        ...
        if (debug) {
            imwrite(
                DebugHelper.getBasePath() + "/tmp/debug_threshold.jpg",
                img_threshold);
        }
        ...
        if (debug) {
            imwrite(
                DebugHelper.getBasePath() + "/tmp/debug_morphology.jpg",
                img_threshold);
        }
        ...
        if (debug) {
            ...
            drawContours(result, contours, -1, new Scalar(0, 0, 255, 255));
            imwrite(
                DebugHelper.getBasePath() + "/tmp/debug_Contours.jpg", result);
        }
