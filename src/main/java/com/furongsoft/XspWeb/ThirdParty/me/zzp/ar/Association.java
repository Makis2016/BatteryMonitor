...
import com.furongsoft.XspWeb.ThirdParty.me.zzp.ar.ex.UndefinedAssociationException;
...
public final class Association
{
    ...
    String assoc(String source, int id)
    {
        ...
        if (isCross()) {
            return String
                .format(template, assoc.target, target, key).concat(" join ")
                .concat(assoc.assoc(source, id));
        }
        else {
            return String
                .format(
                    template.concat(" and %1$s.id = %4$d"), source, target,
                    key, id);
        }
    }
}
