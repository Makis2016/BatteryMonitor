...
public class PlateDetect
{
    ...
    public int plateDetect(final Mat src, Vector<Mat> resultVec)
    {
        ...
        if (getPDDebug()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                String str = "tmp/plate_judge_result_" + Integer
                    .valueOf(i).toString() + ".jpg";
                imwrite(str, img);
            }
        }
     ...
     *         </ul>
     */
    public int plateDetect2(
        final Mat src,
        Vector<Mat> resultVec,
        Vector<RotatedRect> originalVec)
    {
        Vector<RotatedRect> rectVector = new Vector<RotatedRect>();
        Vector<Mat> matVec = plateLocate.plateLocate2(src, rectVector);

        if (0 == matVec.size()) {
            return -1;
        }

        if (0 != plateJudge.plateJudge2(
            matVec, resultVec, rectVector, originalVec)) {
            return -2;
        }
