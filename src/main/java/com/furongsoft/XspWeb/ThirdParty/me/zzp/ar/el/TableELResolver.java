package com.furongsoft.XspWeb.ThirdParty.me.zzp.ar.el;
...
/**
 * <p>
 * <code>me.zzp.ar.Table</code>的EL表达式解析器。 简化<code>Table</code>对象方法调用的方式。
 * </p>
 * 
 * @author redraiment
 * @since 1.0
...
public final class TableELResolver extends ELResolver
{
    /**
     * <p>
     * 支持<code>all</code>、<code>first</code>、<code>last</code>和索引四种查询方式：
     * </p>
     * <ul>
     * <li>all：调用<code>Table#all()</code>。即<code>${User.all}</code>等价于
     * <code>${User.all()}</code></li>
     * <li>first：调用<code>Table#first()</code>。即<code>${User.first}</code>等价于
     * <code>${User.first()}</code></li>
     * <li>last：调用<code>Table#last()</code>。即<code>${User.last}</code>等价于
     * <code>${User.last()}</code></li>
     * <li>索引：调用<code>Table#find(int id)</code>。即<code>${User[1]}</code>等价于
     * <code>${User.find(1)}</code></li>
     * </ul>
     * 
     * @param context
     *            EL表达式上下文
    ...
    /**
     * 如果属性值为<code>all</code>，则返回 {@link java.util.List} 类型； 否则返回
     * {@link me.zzp.ar.Record} 类型。
     * 
     * @param context
     *            EL表达式上下文
     * @param base
